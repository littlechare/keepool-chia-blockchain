...
class FarmerAPI:
    ...
    async def new_proof_of_space(
        ...
        # add littlehow difficulty
        if difficulty > self.farmer.max_difficulty:
            max_pos_per_sp = 5
        ...
        for sp in sps:
            ...
            if computed_quality_string is None:
                ...
                return None
            # add littlehow 如果难度是矿池设定的难度则进行上报
            if difficulty in self.farmer.pool_difficulties:
                for p2_singleton_puzzle_hash, pool_state_dict in self.farmer.pool_state.items():
                            ...
                            async with session.post(f"{pool_url}/partial", data=post_partial_body) as resp:
                                if resp.ok:
                                    ...
                                    if "error_code" in pool_response:
                                        ...
                                        self.farmer.pool_difficulties[0] = new_difficulty
                                else:
    ...
    async def new_signage_point(self, new_signage_point: farmer_protocol.NewSignagePoint):
        ...
        for p2_singleton_puzzle_hash, pool_dict in self.farmer.pool_state.items():
            ...
            if pool_dict["current_difficulty"] is None:
                ...
                continue
            # add littlehow use custom difficulty
        ...
        msgs: List = [msg]
        if self.farmer.sp_count >= self.farmer.frequency:
            message_new = harvester_protocol.NewSignagePointHarvester(
                new_signage_point.challenge_hash,
                self.farmer.pool_difficulties[0],
                new_signage_point.sub_slot_iters,
                new_signage_point.signage_point_index,
                new_signage_point.challenge_chain_sp,
                pool_difficulties,
            )
            msg_new = make_msg(ProtocolMessageTypes.new_signage_point_harvester, message_new)
            self.farmer.log.debug(f'--> change difficulty message :{msg_new}')
            msgs.append(msg_new)
            self.farmer.sp_count = 0
        await self.farmer.server.send_to_all(msgs, NodeType.HARVESTER)
